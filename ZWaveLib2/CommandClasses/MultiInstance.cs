...
namespace ZWaveLib2.CommandClasses
{
    ...
    {
        private static readonly ILogger<MultiInstance> Logger = LogFactory.GetLogger<MultiInstance>();
        ...
        private NodeEvent HandleMultiInstanceEncapReport(ZWaveNode node, byte[] message)
        {
            if (message.Length < 5)
            {
                Logger.LogError(String.Format("MultiInstance encapsulated message ERROR: message is too short: {0}", BitConverter.ToString(message)));
                return null;
            }
            ...
            Logger.LogDebug(String.Format("MultiInstance encapsulated message: CmdClass: {0}; message: {1}", instanceCmdClass, BitConverter.ToString(instanceMessage)));

            var cc = CommandClassFactory.GetCommandClass(instanceCmdClass);
            if (cc == null)
            {
                Logger.LogError(String.Format("Can't find CommandClass handler for command class {0}", instanceCmdClass));
                return null;
            }
        ...
        private NodeEvent HandleMultiChannelEncapReport(ZWaveNode node, byte[] message)
        {
            if (message.Length < 6)
            {
                Logger.LogError(String.Format("MultiChannel encapsulated message ERROR: message is too short: {0}", BitConverter.ToString(message)));
                return null;
            }
            ...
            Logger.LogDebug(String.Format("MultiChannel encapsulated message: CmdClass: {0}; message: {1}", instanceCmdClass, BitConverter.ToString(instanceMessage)));
