...
namespace ZWaveLib2.CommandClasses
{
    ...
    {
        private static readonly ILogger<SecurityData> Logger = LogFactory.GetLogger<SecurityData>();
        ...
        public void GenerateControllerCurrentNonce()
        {
          ...
          Logger.LogDebug("ControllerCurrentNonce: " + BitConverter.ToString(localControllerCurrentNonce));
        ...
        public byte[] GetControllerCurrentNonce(bool clearNonce = false)
        {
            ...
            if (controllerCurrentNonce == null)
            {
                Logger.LogError("ZWave Security Current Nonce is NULL");
        ...
        public byte[] EncryptionKey
        {
            get
            {
                ...
                if (IsAddingNode && !IsNetworkKeySet)
                {
                  ...
                  Logger.LogDebug("In SetupNetworkKey  - in node inclusion mode.");
                }
                else
                {
                ...
                Logger.LogDebug("      networkKey: " + BitConverter.ToString(networkKey));
                Logger.LogDebug("   encryptionKey: " + BitConverter.ToString(encryptionKey));
                Logger.LogDebug("AuthorizationKey: " + BitConverter.ToString(AuthorizationKey));
    ...
    {
        private static readonly ILogger<Security> Logger = LogFactory.GetLogger<Security>();
        ...
        public NodeEvent GetEvent(ZWaveNode node, byte[] message)
        {
            ...
            case (byte)SecurityCommand.SupportedReport:
                Logger.LogDebug("Received COMMAND_SUPPORTED_REPORT for node: " + node.Id);
            ...
            case (byte)SecurityCommand.SchemeReport:
                Logger.LogDebug("Received COMMAND_SCHEME_REPORT for node: " + node.Id + ", " + (startOffset + 1));
                ...
                {
                    Logger.LogDebug("   No common security scheme.  The device will continue as an unsecured node.");
                }
                break;

            case (byte)SecurityCommand.NonceGet:
                Logger.LogDebug("Received COMMAND_NONCE_GET for node: " + node.Id);
            ...
            case (byte)SecurityCommand.MessageEncap:
                Logger.LogDebug("Received COMMAND_MESSAGE_ENCAP for node: " + node.Id);
            ...
            case (byte)SecurityCommand.NonceReport:
                Logger.LogDebug("Received COMMAND_NONCE_REPORT for node: " + node.Id);
            ...
            case (byte)SecurityCommand.MessageEncapNonceGet:
                Logger.LogDebug("Received COMMAND_MESSAGE_ENCAP_NONCE_GET for node: " + node.Id);
            ...
            case (byte)SecurityCommand.NetworkKeySet:
                Logger.LogDebug("Received COMMAND_NETWORK_KEY_SET for node: " + node.Id + ", " + (startOffset + 1));
            ...
            case (byte)SecurityCommand.NetworkKeyVerify:
                Logger.LogDebug("Received COMMAND_NETWORK_KEY_VERIFY for node: " + node.Id + ", " + (startOffset + 1));
            ...
            case (byte)SecurityCommand.SchemeInherit:
                Logger.LogDebug("Received COMMAND_SCHEME_INHERIT for node: " + node.Id);
        ...
        public static void SendMessage(ZWaveNode node, byte[] message)
        {
            ...
            Logger.LogDebug("In sendMsg - SecurityHandler");

            if (message.Length < 7)
            {
                Logger.LogError("Message too short");
            }
            if (message[3] != 0x13)
            {
        ...
        private static bool RequestNonce(ZWaveNode node)
        {
            ...
            Logger.LogDebug("In sendRequestNonce - SecurityHandler");

            if (nodeSecurityData.IsWaitingForNonce)
                return false;

            Logger.LogDebug("In sendRequestNonce - not waiting for Nonce - SecurityHandler");
        ...
        private static bool EncryptMessage(ZWaveNode node, byte[] message)
        {
            ...
            Logger.LogDebug("In EncryptMessage - secure_payload [" + nodeSecurityData.SecurePayload.Count + "]  - " + nodeSecurityData.DeviceNonceTimer.ElapsedMilliseconds);
            ...
            if (payload != null)
            {
                ...
                Logger.LogDebug("authKey " + BitConverter.ToString(nodeSecurityData.AuthorizationKey));
                Logger.LogDebug("EncryptKey " + BitConverter.ToString(nodeSecurityData.EncryptionKey));
                Logger.LogDebug("Input Packet: " + BitConverter.ToString(plaintextmsg));
                Logger.LogDebug("IV " + BitConverter.ToString(initializationVector));
                Logger.LogDebug("encryptedPayload " + BitConverter.ToString(encryptedPayload));
                ...
                Logger.LogDebug("In EncryptMessage - message sent");
        ...
        private static byte[] DecryptMessage(ZWaveNode node, byte[] message, int start)
        {
            ...
            Logger.LogDebug("In DecryptMessage - SecurityHandler");
            if (nodeSecurityData.ControllerNonceTimer.ElapsedMilliseconds > 10000)
            {
                Logger.LogError("Received the nonce  too late'" + nodeSecurityData.ControllerNonceTimer.ElapsedMilliseconds + "' > 10000");
                return null;
            }

            Logger.LogDebug("Message to be decrypted: " + BitConverter.ToString(message));
            ...
            {
                Logger.LogError("currentNonce null");
                return null;
            }

            Logger.LogDebug("currentNonce NOT null");
            ...
            Logger.LogDebug("Message          " + BitConverter.ToString(message));
            Logger.LogDebug("IV               " + BitConverter.ToString(iv));
            Logger.LogDebug("Encrypted Packet " + BitConverter.ToString(encryptedpacket));
            Logger.LogDebug("Decrypted Packet " + BitConverter.ToString(decryptedpacket));
            ...
            {
                Logger.LogDebug("Computed mac " + BitConverter.ToString(mac) + " does not match the provider mac " + BitConverter.ToString(e_mac) + ". Dropping.");
            ...
            if (decryptedpacket[1] == (byte)CommandClass.Security && 1 == 0)
            {
                ...
                Utility.logger.LogDebug("Processing Internally: " + BitConverter.ToString(msg));
            ...
            Logger.LogDebug("Forwarding: " + BitConverter.ToString(msg));

            /* send to the Command Class for Proecssing */
            Logger.LogDebug("Received External Command Class: " + BitConverter.ToString(new byte[] { decryptedpacket[1] }));
//                node.MessageRequestHandler(node.pController, msg);
            Logger.LogDebug("In DecryptMessage - Finished");
        ...
        private static byte[] GenerateAuthentication(byte[] authorizationKey, byte[] data, int start, int length, byte sendingNode, byte receivingNode, byte[] iv)
        {
            ...
            //Utility.logger.LogDebug("Raw Auth (minus IV)" + BitConverter.ToString(buff));
            ...
            //Utility.logger.LogDebug("Computed Auth " + BitConverter.ToString(auth));
