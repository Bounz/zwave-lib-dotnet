...
namespace ZWaveLib2.CommandClasses
{
    public class MultiCmd : ICommandClass
    {
        ...
        public NodeEvent GetEvent(ZWaveNode node, byte[] message)
        {
            ...
            Logger.LogDebug(String.Format("MultiCmd encapsulated message: {0}", BitConverter.ToString(message)));
            ...
            for (i = 0; i < message[2]; i++)
            {
                ...
                Logger.LogDebug(String.Format("Processing message chunk: {0}", BitConverter.ToString(instanceMessage)));
                ...
                if (cc == null)
                {
                    Logger.LogWarning(String.Format("Can't find CommandClass handler for command class {0}", cmdClass));
                    continue;
                }
