...
using Version = ZWaveLib2.CommandClasses.Version;
...
namespace ZWaveLib2
{
    ...
    {
        private static readonly ILogger<ZWaveController> Logger = LogFactory.GetLogger<ZWaveController>();
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ZWaveController"/> class.
        /// </summary>
        public ZWaveController(ILoggerFactory loggerFactory)
        {
            LogFactory.Configure(loggerFactory);
            string codeBase = GetType().Assembly.CodeBase;
            UriBuilder uri = new UriBuilder(codeBase);
            string path = Uri.UnescapeDataString(uri.Path);
            configFolder = Path.GetDirectoryName(path);
            // Setup Serial Port
            serialPort = new SerialPortInput(loggerFactory.CreateLogger<SerialPortInput>());
        ...
        public ZWaveController(string portName, ILoggerFactory loggerFactory) : this(loggerFactory)
        {
            PortName = portName;
        }
        ...
        public ZWaveMessage QueueMessage(ZWaveMessage message)
        {
            ...
            if (node != null && node.SupportCommandClass(CommandClass.WakeUp))
            {
                ...
                {
                    Logger.LogWarning("Node is flagged as sleeping, message will be re-sent on Wake Up (Node={0}, CallbackId={0}, Function={1}, CommandClass={2})", message.NodeId, message.CallbackId.ToString("X2"), message.Function, message.CommandClass);
        ...
        public bool SendMessage(ZWaveMessage message)
        {
            ...
            Logger.LogTrace("Sending Message (Node={0}, CallbackId={1}, Function={2}, CommandClass={3})", pendingRequest.NodeId, pendingRequest.CallbackId.ToString("X2"), pendingRequest.Function, pendingRequest.CommandClass);
            if (serialPort.SendMessage(message.RawData))
            {
                if (!sendMessageAck.WaitOne(ZWaveMessage.SendMessageTimeoutMs))
                {
                    ...
                    Logger.LogWarning("Message timeout (Node={0}, CallbackId={0}, Function={1}, CommandClass={2})", pendingRequest.NodeId, pendingRequest.CallbackId.ToString("X2"), pendingRequest.Function, pendingRequest.CommandClass);
                    if (message.NodeId > 1)
                        UpdateOperationProgress(message.NodeId, NodeQueryStatus.Timeout);
                    //System.Diagnostics.LogDebugger.Break();
                }
            }
            else
            {
                SetQueryStage(QueryStage.Error);
                Logger.LogWarning("Controller status error (Node={0}, CallbackId={0}, Function={1}, CommandClass={2})", pendingRequest.NodeId, pendingRequest.CallbackId.ToString("X2"), pendingRequest.Function, pendingRequest.CommandClass);
            }
            pendingRequest = null;
            #region Debug
            stopWatch.Stop();
            Logger.LogTrace("[[[ END REQUEST ]]] took {0} ms", stopWatch.ElapsedMilliseconds);
            #endregion
        ...
        public void Discovery()
        {
            ...
            if (!discoveryRunning)
            {
                ...
                foreach (ZWaveNode zn in nodeList)
                {
                    ...
                    Logger.LogTrace("Getting protocol info for node {0}", zn.Id);
                ...
                foreach (ZWaveNode zn in nodeList)
                {
                    ...
                    Logger.LogTrace("Querying/Updating node {0}", zn.Id);
            ...
            {
                Logger.LogWarning("Discovery already running");
            }
        }
        ...
        public void HealNetwork()
        {
            ...
            if (!healRunning)
            {
                ...
                foreach (ZWaveNode zn in nodeList)
                {
                    ...
                    Logger.LogTrace("Healing node {0}", zn.Id);
            ...
            {
                Logger.LogWarning("Heal already running");
            }
        }
            ...
            if (zn.SupportCommandClass(CommandClass.Version))
            {
                foreach (var cmdClass in zn.CommandClasses)
                {
                    ...
                    if (cmdClass.CommandClass != CommandClass.NotSet && cmdClass.Version == -1)
                        Version.Get(zn, cmdClass.CommandClass).Wait();
                }
            }
        }
            ...
            while (!disposing)
            {
                while (queuedMessages.Count > 0 && !busyReceiving && !disposing)
                {
                    ...
                    if (controllerStatus == ControllerStatus.Ready)
                    {
                        ...
                        {
                            msg.ResendCount++;
                            Logger.LogWarning("Could not deliver message to Node {0} (CallbackId={1}, Retry={2})", msg.NodeId, msg.CallbackId.ToString("X2"), msg.ResendCount);
                        ...
                        {
                            Logger.LogWarning("Delivery of message to Node {0} failed (CallbackId={1}).", msg.NodeId, msg.CallbackId.ToString("X2"));
                            if (msg.NodeId > 1)
                            {
                                ...
                                {
                                    Logger.LogWarning("Node is flagged as sleeping, message will be re-sent on Wake Up (Node={0}, CallbackId={0}, Function={1}, CommandClass={2})", msg.NodeId, msg.CallbackId.ToString("X2"), msg.Function, msg.CommandClass);
                    ...
                    {
                        Logger.LogWarning("Controller not ready, delivery of message to Node {0} failed (CallbackId={1}).", msg.NodeId, msg.CallbackId.ToString("X2"));
                        if (msg.NodeId > 1)
                            UpdateOperationProgress(msg.NodeId, NodeQueryStatus.Error);
                    }
        ...
        private void ReceiveMessage(ZWaveMessage msg)
        {
            ...
            {
                Logger.LogWarning("Repeated message discarded.");
                return;
            }
            ...
            case MessageType.Request:
                ...
                case ZWaveFunction.RequestNodeNeighborsUpdate:
                    ...
                    default:
                        Logger.LogWarning("Unhandled Node Neighbor Update request: {0}", BitConverter.ToString(rawData));
                        break;
                ...
                case ZWaveFunction.ApplicationCommandHandler:
                    ...
                    {
                        Logger.LogError("Unknown node id {0}", rawData[5]);
                    }
                    break;
                ...
                default:
                    Logger.LogWarning("Unhandled request message: {0}", BitConverter.ToString(rawData));
                    break;
            ...
            case MessageType.Response:
                ...
                case ZWaveFunction.GetHomeId:
                    if (rawData.Length > 7)
                    {
                        ...
                        Logger.LogInformation("Home Id is {0}, Controller node id is {1}", BitConverter.ToString(homeId), nodeId);
                        // TODO: complete this code
                    }
                    else
                    {
                        Logger.LogWarning("Could not read Home Id.");
                    }
                    break;
                ...
                case ZWaveFunction.GetSucNodeId:
                    // TODO: complete this code
                    Logger.LogWarning("Response handling for {0} not implemented yet!", msg.Function);
                    break;
                ...
                case ZWaveFunction.GetRoutingInfo:
                    var routingInfo = Utility.Utility.ExtractRoutingFromBitMask(rawData);
                    ...
                    {
                        Logger.LogWarning("No routing nodes reported.");
                    }
                    break;

                default:
                    Logger.LogWarning("Unhandled response message: {0}", BitConverter.ToString(rawData));
                    break;
            ...
            default:
                Logger.LogWarning("Unhandled message type: {0}", BitConverter.ToString(rawData));
                break;
            }
        ...
        {
            Logger.LogTrace(stage.ToString());
            currentStage = stage;
        ...
        private void UpdateQueryStage(ZWaveMessage zm)
        {
            ...
            {
                //Utility.logger.LogTrace("Query Stage {0} Type {1} Function {2}={3} Node {4}={5} Callback {6}={7}", currentStage, zm.Type, zm.Function, currentMessage.Function, zm.NodeId, currentMessage.NodeId, zm.CallbackId, currentMessage.CallbackId);
                switch (currentStage)
                {
        ...
        private void ProcessMessage(ZWaveMessage zm)
        {
            if (zm.Header == FrameHeader.SOF)
            {
                ...
                {
                    SendNack();
                    Logger.LogWarning("Bad message checksum");
                }
            }
            ...
            {
                Logger.LogWarning("Unhandled message type: {0}", BitConverter.ToString(zm.RawData));
            }
        }
            ...
            else if (eventData.Parameter == EventParameter.SecurityNodeInformationFrame)
            {
                ...
                foreach (byte nodeInfo in node.SecuredNodeInformationFrame)
                {
                    ...
                    if (nodeInfo == (byte)0xEF)
                        break;
                    node.NodeInformationFrame = Utility.Utility.AppendByteToArray(node.NodeInformationFrame, nodeInfo);
                }
        ...
        private void ParseSerialData(byte[] message)
        {
            if (serialBuffer != null)
            {
                ...
                message = merged;
                serialBuffer = null;
                Logger.LogTrace("Merged buffer to message: {0}", BitConverter.ToString(message));
            }
            ...
            else if (message.Length > 1)
            {
                ...
                else if (header == FrameHeader.SOF && message.Length < msgLength + 2)
                {
                    ...
                    Logger.LogTrace("Expected message length is {0}, currently received length is {1}", msgLength + 2, message.Length);
                    return;
                }
            }
        ...
        {
            var nodes = Utility.Utility.ExtractNodesFromBitMask(receivedMessage);
        ...
        {
            Logger.LogDebug("NodeUpdated (NodeId={0}, Parameter={1}, Value={2})", args.NodeId, args.Event.Parameter, args.Event.Value);
            if (NodeUpdated != null)
        ...
        {
            Logger.LogDebug(args.Status.ToString());
        ...
        {
            Logger.LogDebug("{0} {1}", args.NodeId, args.Status);
        ...
        protected virtual void OnControllerStatusChanged(ControllerStatusEventArgs args)
        {
            ...
            Logger.LogDebug("{0}", controllerStatus);
